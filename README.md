...
## Text-first intake and recurring sessions

Northstar V2 accepts pasted academic and timetable text. Academic Inbox stores the original text, produces validated reviewable proposals, and performs no academic mutation until an owner explicitly approves a proposal. Timetable intake similarly creates an owned review draft from pasted text; confirmation remains transactional and idempotent.
...
Pasted registered-course text and manual session entry are supported. Ambiguous fields remain reviewable and no imported information is represented as officially verified.
...
Run `npm run test:run` for parser, validation, ownership, transaction, conflict, navigation, and composable-context coverage. Run `npm run build` to verify the production bundle.
