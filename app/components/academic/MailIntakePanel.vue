    ...
    <div v-else-if="records.length" class="mail-reviews">
      <article v-for="intake in records" :key="intake.id" class="mail-review">
        ...
        <div v-if="['OPPORTUNITY','EVENT'].includes(intake.classification)" class="mail-extracted">
          ...
          <dl v-else><div><dt>Organisation</dt><dd>{{ opportunity(intake).organisation || 'Unresolved' }}</dd></div><div><dt>Category</dt><dd>{{ opportunity(intake).category ? label(opportunity(intake).category) : 'Unresolved' }}</dd></div><div><dt>Event</dt><dd>{{ opportunity(intake).startAt ? new Date(opportunity(intake).startAt).toLocaleString() : 'Not stated' }}<template v-if="opportunity(intake).endAt"> – {{ new Date(opportunity(intake).endAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }}</template></dd></div><div><dt>Venue</dt><dd>{{ opportunity(intake).location || 'Not stated' }}</dd></div><div><dt>Deadline</dt><dd>{{ opportunity(intake).deadline ? new Date(opportunity(intake).deadline).toLocaleString() : opportunity(intake).deadlineSourceText || 'Not stated' }}</dd></div><div><dt>Eligibility</dt><dd>{{ opportunity(intake).eligibilityText || 'Unresolved' }}</dd></div><div><dt>Requirements</dt><dd>{{ opportunity(intake).requirements || 'Not stated' }}</dd></div><div><dt>Benefits</dt><dd>{{ opportunity(intake).benefits || 'Not stated' }}</dd></div><div><dt>Application</dt><dd><a v-if="opportunity(intake).applicationUrl" :href="opportunity(intake).applicationUrl" target="_blank" rel="noopener noreferrer">Detected registration form</a><span v-else>Unresolved</span></dd></div></dl>
        </div>

        <div v-if="['ACTION_REQUIRED','ACADEMIC_ADMIN'].includes(intake.classification)" class="mail-extracted"><strong v-if="intake.classification === 'ACTION_REQUIRED'">{{ admin(intake).title || intake.subject || 'Action required' }}</strong><dl><div v-if="admin(intake).organisation"><dt>Organisation</dt><dd>{{ admin(intake).organisation }}</dd></div><div v-if="admin(intake).selectionStage"><dt>Stage</dt><dd>{{ label(admin(intake).selectionStage) }}</dd></div><div v-if="admin(intake).moduleCode"><dt>Module</dt><dd>{{ admin(intake).moduleCode }}</dd></div><div><dt>Type</dt><dd>{{ admin(intake).academicSubtype ? label(admin(intake).academicSubtype) : 'Academic notice' }}</dd></div><div v-if="admin(intake).eventDate"><dt>Class date</dt><dd>{{ admin(intake).eventDate }}</dd></div><div v-if="admin(intake).venue"><dt>Venue</dt><dd>{{ admin(intake).venue }}</dd></div><div><dt>Action</dt><dd>{{ admin(intake).requiresAction ? admin(intake).actionRequired : 'No explicit action required' }}</dd></div><div><dt>Deadline</dt><dd>{{ admin(intake).exactDeadline ? new Date(admin(intake).exactDeadline).toLocaleString([], { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'None detected' }}</dd></div></dl><div v-if="admin(intake).possibleRelatedOpportunity" class="mail-linked">Possible related opportunity · {{ admin(intake).possibleRelatedOpportunity.title }}</div><div v-if="admin(intake).resources?.length" class="mail-resources"><strong>Resources</strong><a v-for="resource in admin(intake).resources" :key="`${resource.label}-${resource.url}`" :href="resource.url || undefined" :target="resource.url ? '_blank' : undefined" rel="noopener noreferrer">{{ resource.label }} · {{ label(resource.type) }}</a></div></div>
        ...
        <div v-if="intake.status === 'NEW'" class="v2-inline-actions mail-actions">
          ...
          <template v-else-if="intake.classification === 'ACTION_REQUIRED'"><UButton size="xs" :loading="saving" @click="decide(intake, 'task')">Create task</UButton></template>
          <template v-else-if="intake.classification === 'ACADEMIC_ADMIN'"><UButton size="xs" :loading="saving" @click="decide(intake, 'task')">Create task</UButton><UButton size="xs" color="neutral" variant="outline" :loading="saving" @click="decide(intake, 'note')">Save note</UButton></template>
