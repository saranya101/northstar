<script setup>
import { CLASS_SESSION_TYPES, DAYS_OF_WEEK, SESSION_DELIVERY_MODES, SESSION_RECURRENCES } from '~~/shared/schemas/timetable'
  ...
  <UModal :open="open" :title="session ? 'Edit class session' : 'Add class session'" description="Recurring sessions are private to your enrolment." @update:open="value => emit('update:open', value)">
    <template #body>
      <form id="session-form" class="module-form" @submit.prevent="submit">
        ...
        <div class="module-field"><label for="session-delivery">Delivery mode</label><span class="delivery-control"><UIcon :name="deliveryModeIcon(form.deliveryMode)" /><select id="session-delivery" v-model="form.deliveryMode"><option v-for="item in SESSION_DELIVERY_MODES" :key="item" :value="item">{{ deliveryModeLabel(item) }}</option></select></span></div>
