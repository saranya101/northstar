...
export function useTimetable() {
  ...
  function clear() { nuxtApp[TIMETABLE_REQUESTS].clear(); state.value = null; imports.value = {}; ownerId.value = null; pending.value = 0; saving.value = false; clearErrors() }
  ...
  async function createImport(body) { const result = await mutate('/api/timetable/imports', { method: 'POST', body }, 'Unable to create the import review.'); if (result) imports.value[result.id] = result; return result }
