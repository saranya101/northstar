...
describe('opportunity card presentation', () => {
  it('keeps deadline and event date selection explicitly separate', async () => {
    ...
    expect(card).toContain('opportunity-card__secondary-meta')
    expect(card).toContain('<dt>Location</dt>')
    expect(card).toContain('<dt>Mode</dt>')
    expect(card.indexOf('opportunity-card__secondary-meta'))
      .toBeGreaterThan(card.indexOf('v-show="expanded"'))
