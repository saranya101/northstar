...
describe('task engine UI contract', () => {
  it('adds an authenticated task route as a primary V2 navigation item', () => { const page = read('app/pages/app/tasks.vue'); expect(page).toContain("middleware: ['auth', 'onboarded']"); expect(read('app/layouts/app.vue').match(/label: 'Tasks'/g)).toHaveLength(1) })
